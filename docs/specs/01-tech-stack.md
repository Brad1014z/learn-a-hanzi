...
> **Status:** ACCEPTED (reviewed 2026-07-05; amended 2026-07-09 — pre-generated audio
> moved up to the content milestone M2, ingest tooling may call the Claude API +
> cloud TTS at build time; see `02`/`08`)
...
- **No network persistence layer in MVP.** Sync is a cloud-layer concern (milestone M4, `12`)
...
- **MVP implementation: Android `TextToSpeech`** with `Locale.SIMPLIFIED_CHINESE`
  ...
  characters.
- **Quality upgrade (milestone M2 — moved up 2026-07-09): pre-generated audio as data** —
  the ingest pipeline calls a cloud TTS once per curriculum character/word/sentence and
  ships the clips; a `PregenAudioSpeechService` plays them, falling back to device TTS
  for anything uncovered. No runtime audio backend (see `02`/`12`).
...
- No Google services (Firebase, etc.) **in the MVP builds**. Milestone M4 introduces Firebase
  as the optional cloud layer — see below and `12`.

## Cloud-layer additions (milestone M4) — NOT in earlier builds

> Recorded here so the stack decision is set; nothing below is compiled into earlier
...
- **Content generation (milestone M2, ingest-side):** a cloud TTS API and the Claude
  API (LLM example sentences, human-reviewed — see `02`) are used by `:data-ingest`
  only, at build time — never called from the app; if runtime TTS is ever needed, a
  Cloudflare Worker proxy with edge caching is the designated shape (`12`).
...
- [ ] Confirm minSdk 26 doesn't exclude a meaningful share of the audience — check Play
      device stats at publish time (Publish milestone).
